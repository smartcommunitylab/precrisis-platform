...
if st.session_state.current_city != 'Limassol':
    ...
    #         """)
    st.write("""
            This submodule analyzes street images to estimate their perceived safety. The interactive map in this submodule displays available locations within the user-selected city. Each point on the map represents an image and is color-coded:
            
             - :green[Green] points indicate locations the AI model perceives as safe.
             - :red[Red] points represent locations considered unsafe.    
             
             Users can hover over any point to view descriptive keywords generated by the AI model that explain its perception. For instance, safe areas may be associated with positive attributes like cleanliness, orderliness, or the absence of visible threats or neglect.
             """)
    ...
    tooltip_template = '{v} <br> <img src="https://precrisis.smartcommunitylab.it/show/plot/images/'+st.session_state.current_city.lower()+'/{img}.jpeg" width="400">' if st.session_state.current_city != 'Limassol' else '{v}'
    r = pdk.Deck(
        layers=[geojson],
